...
# in reverse order. Complete the steps to combine them into one list as follows: the contents of Drew's list, followed
...
# Followed by the elements of list1 in reverse order
    ...
    [newlist.insert(len(newlist),x) for x in reversed(list1)]
...
def combine_guests(guests1, guests2):
  ...
  # only once, and the value from guests1 taking precedence
    result = {}
    for name,noOfGuests in guests1.items():
        for name2, noOfGuests2 in guests2.items():
            if name == name2:
                result[name] = noOfGuests
                continue
